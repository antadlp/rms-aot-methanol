...
def estandar_npt_000(**kwargs):
    ...
    INTEGRATOR = df_mdp_integrator['integrator'].loc[0]
    ...
    path = '../data/inputs/mdp_options/input_variable_rlist.pkl'
    df_mdp_rlist = pd.read_pickle(path)
    RLIST = df_mdp_rlist['rlist'].loc[0]
    rlist = kwargs.get('rlist', RLIST)
    RLIST = rlist
    dic['rlist'][0] = RLIST

    path = '../data/inputs/mdp_options/input_variable_fourierspacing.pkl'
    df_mdp_fourierspacing = pd.read_pickle(path)
    FOURIERSPACING = df_mdp_fourierspacing['fourierspacing'].loc[1]
    ...
    TC_GRPS = df_mdp_tc_grps['tc-grps'].loc[1]
    ...
    path = '../data/inputs/mdp_options/input_variable_tau-p.pkl'
    df_mdp_tau_p = pd.read_pickle(path)
    TAU_P = df_mdp_tau_p['tau-p'].loc[0]
    ...
    path = '../data/inputs/mdp_options/input_variable_nh-chain-length.pkl'
    df_mdp_nh_chain_length = pd.read_pickle(path)
    NHCHAINLENGTH = df_mdp_nh_chain_length['nh-chain-length'].loc[0]
    nhchainlength = kwargs.get('nh_chain_length', NHCHAINLENGTH)
    NHCHAINLENGTH = nhchainlength
    dic['nh-chain-length'][0] = NHCHAINLENGTH

    path = '../data/inputs/mdp_options/input_variable_pcoupl.pkl'
    df_mdp_pcoupl = pd.read_pickle(path)
    PCOUPL = df_mdp_pcoupl['pcoupl'].loc[2]
    ...
    path = '../data/inputs/mdp_options/input_variable_compressibility.pkl'
    df_mdp_compressibility = pd.read_pickle(path)
    COMPRESSIBILITY = df_mdp_compressibility['compressibility'].loc[0]
    compressibility = kwargs.get('compressibility', COMPRESSIBILITY)
    COMPRESSIBILITY = compressibility
    dic['compressibility'][0] = COMPRESSIBILITY

    path = '../data/inputs/mdp_options/input_variable_ref-p.pkl'
    df_mdp_ref_p = pd.read_pickle(path)
    REF_P = df_mdp_ref_p['ref-p'].loc[0]
    ref_p = kwargs.get('ref_p', REF_P)
    REF_P = ref_p
    dic['ref-p'][0] = REF_P
